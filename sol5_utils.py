...
def list_images(path, use_shuffle=True):
    ...
    images = list(map(lambda x: os.path.join(path, x),
                      filter(is_image, os.listdir(path))))
...
def images_for_super_resolution():
    """Returns a list of image paths to be used for image super-resolution in
    Ex5"""
    return list_images(relpath('image_dataset/train'), True)
...
def motion_blur_kernel(kernel_size, angle):
    ...
    if angle < 0 or angle > np.pi:
        raise ValueError(
            'angle must be between 0 (including) and pi (not including)')
